...
var commands = Config.env({
    ...
    config = index && commands[index].config || { branchName: 'master' };
...
exports.updateFiles = function (params, cb) {
    ...
    console.log('requireGitBranchCommand.updateFiles', 'required:', config.branchName, 'current:', currentBranch);

    if (currentBranch !== config.branchName) {
        cb('Required branch ' + config.branchName+ ', instead found: ' + currentBranch + '. ');
    } else {
        process.nextTick(cb);
    }
